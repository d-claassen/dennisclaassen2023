 ...
 */
const { getArgFromCLI, hasArgInCLI } = require( './node_modules/@wordpress/scripts/utils' );
 ...
 *
 * The license string can be a single license, or an SPDX-compatible "OR"
 * license string. eg, "(MIT OR Zlib)".
 *
 * @param {string} allowedLicense The license that's allowed.
 * @param {string} licenseType    The license string to check.
 *
 * @return {boolean} true if the licenseType matches the allowedLicense, false
 *     if it doesn't.
 */
 ...
 *
 * This function is invoked when the licenses are a conjunctive ("AND") list of
 * licenses. In that case, the software is only compatible if all of the
 * licenses in the list are compatible.
 *
 * @param {Array} packageLicenses    The licenses that a package is licensed
 *     under.
 * @param {Array} compatibleLicenses The list of compatible licenses.
 *
 * @return {boolean} true if all of the packageLicenses appear in
 *     compatibleLicenses.
 */
